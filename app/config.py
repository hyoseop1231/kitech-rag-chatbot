...
class Settings:
    ...
    # Ollama LLM settings (legacy/fallback)
    ...
    LLM_NUM_PREDICT_MULTIMODAL: int = int(
        os.getenv("LLM_NUM_PREDICT_MULTIMODAL", "3072")
    )

    # OCR correction specific LLM settings (경량 모델 사용)
    OCR_LLM_MODEL: str = os.getenv(
        "OCR_LLM_MODEL", "google/gemini-2.0-flash-001"
    )  # OpenRouter 모델
    OCR_LLM_TIMEOUT: int = int(os.getenv("OCR_LLM_TIMEOUT", "60"))  # 짧은 타임아웃
    OCR_LLM_TEMPERATURE: float = float(
        os.getenv("OCR_LLM_TEMPERATURE", "0.3")
    )  # 낮은 창의성
    OCR_LLM_NUM_PREDICT: int = int(
        os.getenv("OCR_LLM_NUM_PREDICT", "1024")
    )  # 짧은 응답
    ...
    OCR_CORRECTION_ENABLED: bool = (
        os.getenv("OCR_CORRECTION_ENABLED", "True").lower() == "true"
    )
    OCR_CORRECTION_USE_LLM: bool = (
        os.getenv("OCR_CORRECTION_USE_LLM", "True").lower() == "true"
    )
    ...
    SIMILARITY_THRESHOLD: float = float(
        os.getenv("SIMILARITY_THRESHOLD", "0.9")
    )  # Higher = more strict
    ...
    ENABLE_EXTERNAL_ACCESS: bool = (
        os.getenv("ENABLE_EXTERNAL_ACCESS", "True").lower() == "true"
    )
    ...
    ENABLE_PARALLEL_SEARCH: bool = (
        os.getenv("ENABLE_PARALLEL_SEARCH", "True").lower() == "true"
    )
    ENABLE_ASYNC_LLM: bool = os.getenv("ENABLE_ASYNC_LLM", "True").lower() == "true"
    CONTEXT_COMPRESSION_MAX_TOKENS: int = int(
        os.getenv("CONTEXT_COMPRESSION_MAX_TOKENS", "2000")
    )

    # File processing limits
    MAX_CONCURRENT_FILE_PROCESSING: int = int(
        os.getenv("MAX_CONCURRENT_FILE_PROCESSING", "5")
    )
